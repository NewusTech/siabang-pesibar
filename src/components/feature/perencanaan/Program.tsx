...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.program.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.program.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, bidangId: router[1] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, bidangId: router[1] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('bidangId', item.bidangId)
      }
    } else {
      form.reset()
    }
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Program' : 'Tambah Program'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const Program = () => {
  ...
  const { data, refetch } = api.program.getbyid.useQuery({ bidangId: slug[1]})
  const { mutateAsync, isLoading } = api.program.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Urusan", link: "/master/perencanaan" }, { name: "Bidang", link: `/master/perencanaan/${slug[0]}`}, { name: "Program" }]} />
