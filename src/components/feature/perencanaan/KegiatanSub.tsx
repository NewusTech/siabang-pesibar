...
const columnHelper = createColumnHelper<SubKegiatan>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.subKegiatan.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.subKegiatan.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, kegiatanId: router[3] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, kegiatanId: router[3] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('kegiatanId', item.kegiatanId)
      }
    } else {
      form.reset()
    }
...
const KegiatanSub = () => {
  ...
  const { data, refetch } = api.subKegiatan.getbyid.useQuery({ kegiatanId: slug[3]})
  const { mutateAsync, isLoading } = api.subKegiatan.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[ { name: "Urusan", link: "/master/perencanaan" }, { name: "Bidang", link: `/master/perencanaan/${slug[0]}`}, { name: "Program", link: `/master/perencanaan/${slug[0]}/${slug[1]}` }, { name: "Kegiatan", link: `/master/perencanaan/${slug[0]}/${slug[1]}/${slug[2]}` }, { name: "Sub Kegiatan" }]} />
