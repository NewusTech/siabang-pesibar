...
const columnHelper = createColumnHelper<Kegiatan>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.kegiatan.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.kegiatan.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, programId: router[2] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, programId: router[2] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('programId', item.programId)
      }
    } else {
      form.reset()
    }
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Kegiatan' : 'Tambah Kegiatan'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const Kegiatan = () => {
  ...
  const { data, refetch } = api.kegiatan.getbyid.useQuery({ programId: slug[2]})
  const { mutateAsync, isLoading } = api.kegiatan.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Urusan", link: "/master/perencanaan" }, { name: "Bidang", link: `/master/perencanaan/${slug[0]}`}, { name: "Program", link: `/master/perencanaan/${slug[0]}/${slug[1]}` }, { name: "Kegiatan" }]} />
