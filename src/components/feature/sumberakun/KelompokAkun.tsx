...
const columnHelper = createColumnHelper<kelompokAkun>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.kelompokAkun.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.kelompokAkun.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, sumberAkunId: router[0] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, sumberAkunId: router[0] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('sumberAkunId', item.sumberAkunId)
      }
    } else {
      form.reset()
    }
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Kelompok Akun' : 'Tambah Kelompok Akun'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const KelompokAkun = () => {
  ...
  const { data, refetch } = api.kelompokAkun.get.useQuery({ sumberAkunId: slug[0]})
  const { mutateAsync, isLoading } = api.kelompokAkun.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Sumber Akun", link: "/master/akun" }, { name: "Kelompok Akun" }]} />
          <p className="font-semibold">List Kelompok Akun</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
