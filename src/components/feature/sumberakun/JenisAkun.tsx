...
const columnHelper = createColumnHelper<JenisAkun>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.jenisAkun.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.jenisAkun.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, kelompokAkunId: router[1] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, kelompokAkunId: router[1] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('kelompokAkunId', item.kelompokAkunId)
      }
    } else {
      form.reset()
    }
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Jenis Akun' : 'Tambah Jenis Akun'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const JenisAkun = () => {
  ...
  const { data, refetch } = api.jenisAkun.get.useQuery({ kelompokAkunId: slug[1]})
  const { mutateAsync, isLoading } = api.jenisAkun.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Sumber Akun", link: "/master/akun" }, { name: "Kelompok Akun", link: `/master/akun/${slug[0]}`}, { name: "Jenis Akun" }]} />
          <p className="font-semibold">List Jenis Akun</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
