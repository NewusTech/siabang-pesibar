...
const columnHelper = createColumnHelper<ObjekAkun>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.objekAkun.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.objekAkun.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, jenisAkunId: router[2] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, jenisAkunId: router[2] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('jenisAkunId', item.jenisAkunId)
      }
    } else {
      form.reset()
    }
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Objek Akun' : 'Tambah Objek Akun'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const ObjekAkun = () => {
  ...
  const { data, refetch } = api.objekAkun.get.useQuery({ jenisAkunId: slug[2]})
  const { mutateAsync, isLoading } = api.objekAkun.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Sumber Akun", link: "/master/akun" }, { name: "Kelompok Akun", link: `/master/akun/${slug[0]}`}, { name: "Jenis Akun", link: `/master/akun/${slug[0]}/${slug[1]}` }, { name: "Objek Akun" }]} />
          <p className="font-semibold">List Objek Akun</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
