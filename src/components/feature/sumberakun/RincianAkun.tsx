...
const columnHelper = createColumnHelper<RincianAkun>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.rincianObjekAkun.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.rincianObjekAkun.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, objekAkunId: router[3] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, objekAkunId: router[3] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('objekAkunId', item.objekAkunId)
      }
    } else {
      form.reset()
    }
...
const RincianAkun = () => {
  ...
  const { data, refetch } = api.rincianObjekAkun.get.useQuery({ objekAkunId: slug[3]})
  const { mutateAsync, isLoading } = api.rincianObjekAkun.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Sumber Akun", link: "/master/akun" }, { name: "Kelompok Akun", link: `/master/akun/${slug[0]}`}, { name: "Jenis Akun", link: `/master/akun/${slug[0]}/${slug[1]}` }, { name: "Objek Akun", link: `/master/akun/${slug[0]}/${slug[1]}/${slug[2]}` }, { name: "Rincian Objek Akun" }]} />
          <p className="font-semibold">List Rincian Objek Akun</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
