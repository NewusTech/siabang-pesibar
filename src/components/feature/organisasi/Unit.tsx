...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.unit.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.unit.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, organisasiId: router[0] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, organisasiId: router[0] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('organisasiId', item.organisasiId)
      }
    } else {
      form.reset()
    }
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Unit' : 'Tambah Unit'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const Unit = () => {
  ...
  const { data, refetch } = api.unit.get.useQuery({ organisasiId: slug[0]})
  const { mutateAsync, isLoading } = api.unit.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Organisasi", link: "/master/organisasi" }, { name: "Unit" }]} />
          <p className="font-semibold">List Unit</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
