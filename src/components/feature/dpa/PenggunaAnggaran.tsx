...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.anggaran.addPengguna.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.anggaran.updatePengguna.useMutation()
...
const PenggunaAnggaran = () => {
  const router = useRouter()
  const id = router.query.slug![0]!

  const [modalState, setModalState] = useState({ isOpen: false, item: null })
  const [deleteModal, setDeleteModal] = useState<null | string>(null)
  const { data: pengguna, refetch } = api.anggaran.getPengguna.useQuery({ id })
  const { data: anggaran } = api.anggaran.get.useQuery({ id })
  const { mutateAsync } = api.anggaran.deletePenggunaAnggaran.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        ...
        <MultiStep data={dpaMultistep} type="anggaran" onNext={() => {
          router.push(`/anggaran/dpa/${id}/rincian/sub-kegiatan/rencana/pengguna/ttd`)
        }} >
          <div className="my-5">
            ...
            <DataTable
              ...
              filterBy="nama"
              option={
                <Button onClick={handleOpenAddModal} disabled={isFormDisabled}>Tambah</Button>
              }
            />
          </div>
          <DeleteModal
...
export default PenggunaAnggaran