...
const columnHelper = createColumnHelper<JenisDana>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.jenisDana.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.jenisDana.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, kelompokDanaId: router[1] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, kelompokDanaId: router[1] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('kelompokDanaId', item.kelompokDanaId)
      }
    } else {
      form.reset()
    }
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Jenis Dana' : 'Tambah Jenis Dana'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const JenisDana = () => {
  ...
  const { data, refetch } = api.jenisDana.get.useQuery({ kelompokDanaId: slug[1]})
  const { mutateAsync, isLoading } = api.jenisDana.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Sumber Dana", link: "/master/sumber-dana" }, { name: "Kelompok Dana", link: `/master/sumber-dana/${slug[0]}`}, { name: "Jenis Dana" }]} />
          <p className="font-semibold">List Jenis Dana</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
