...
const columnHelper = createColumnHelper<ObjekDana>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.objekDana.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.objekDana.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, jenisDanaId: router[2] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, jenisDanaId: router[2] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('jenisDanaId', item.jenisDanaId)
      }
    } else {
      form.reset()
    }
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Objek Dana' : 'Tambah Objek Dana'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const ObjekDana = () => {
  ...
  const { data, refetch } = api.objekDana.get.useQuery({ jenisDanaId: slug[2]})
  const { mutateAsync, isLoading } = api.objekDana.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Sumber Dana", link: "/master/sumber-dana" }, { name: "Kelompok Dana", link: `/master/sumber-dana/${slug[0]}`}, { name: "Jenis Dana", link: `/master/sumber-dana/${slug[0]}/${slug[1]}` }, { name: "Objek Dana" }]} />
          <p className="font-semibold">List Objek Dana</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
