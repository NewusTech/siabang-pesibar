...
const columnHelper = createColumnHelper<kelompokDana>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.kelompokDana.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.kelompokDana.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, sumberDanaId: router[0] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, sumberDanaId: router[0] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('sumberDanaId', item.sumberDanaId)
      }
    } else {
      form.reset()
    }
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Kelompok Dana' : 'Tambah Kelompok Dana'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const KelompokDana = () => {
  ...
  const { data, refetch } = api.kelompokDana.get.useQuery({ sumberDanaId: slug[0]})
  const { mutateAsync, isLoading } = api.kelompokDana.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Sumber Dana", link: "/master/sumber-dana" }, { name: "Kelompok Dana" }]} />
          <p className="font-semibold">List Kelompok Dana</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
