...
const columnHelper = createColumnHelper<RincianDana>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.rincianObjekDana.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.rincianObjekDana.update.useMutation()
  ...
  const handleSave = async (values: z.infer<typeof formSchema>) => {
    if (isEditMode) {
      try {
        const result = await update({ id: item.id, name: values.name, kode: values.kode, objekDanaId: router[3] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
      ...
      try {
        const result = await mutateAsync({ name: values.name, kode: values.kode, objekDanaId: router[3] })
        if (result.ok) {
          refetch()
          form.reset()
          onClose();
  ...
  useEffect(() => {
    if (isEditMode) {
      if (item) {
        ...
        form.setValue('objekDanaId', item.objekDanaId)
      }
    } else {
      form.reset()
    }
...
const RincianDana = () => {
  ...
  const { data, refetch } = api.rincianObjekDana.get.useQuery({ objekDanaId: slug[3]})
  const { mutateAsync, isLoading } = api.rincianObjekDana.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Sumber Dana", link: "/master/sumber-dana" }, { name: "Kelompok Dana", link: `/master/sumber-dana/${slug[0]}`}, { name: "Jenis Dana", link: `/master/sumber-dana/${slug[0]}/${slug[1]}` }, { name: "Objek Dana", link: `/master/sumber-dana/${slug[0]}/${slug[1]}/${slug[2]}` }, { name: "Rincian Objek Dana" }]} />
          <p className="font-semibold">List Rincian Objek Dana</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
