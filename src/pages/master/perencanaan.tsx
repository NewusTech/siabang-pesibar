...
const columnHelper = createColumnHelper<Urusan>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.urusan.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.urusan.update.useMutation()
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Urusan' : 'Tambah Urusan'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const Page = () => {
  ...
  const { data, refetch } = api.urusan.get.useQuery()
  const { mutateAsync, isLoading } = api.urusan.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Urusan" }]} />
          <p className="font-semibold">List Urusan</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
            {
              ...
              header: () => 'KODE',
              cell: ({ row }: { row: any }) => (
                <Link href={`perencanaan/${row.original.id}`} className="text-blue-500 hover:underline">
                  {row.original.kode}
                </Link>
              ),
            },
