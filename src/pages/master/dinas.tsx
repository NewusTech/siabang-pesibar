...
const columnHelper = createColumnHelper<Dinas>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.dinas.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.dinas.update.useMutation()
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah OPD' : 'Tambah OPD'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
...
const Page = () => {
  ...
  const { data, refetch } = api.dinas.get.useQuery()
  const { mutateAsync, isLoading } = api.dinas.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "OPD" }]} />
          <p className="font-semibold">List OPD</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
