...
const columnHelper = createColumnHelper<SumberDana>()
...
const AddEditModal = ({
  ...
  const { mutateAsync, isLoading } = api.sumberDana.add.useMutation()
  const { mutateAsync: update, isLoading: isUpdateLoading } = api.sumberDana.update.useMutation()
  ...
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isEditMode ? 'Ubah Sumber Dana' : 'Tambah Sumber Dana'}</AlertDialogTitle>
        </AlertDialogHeader>
        <div>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="nama sumber Dana" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
...
const Page = () => {
  ...
  const { data, refetch } = api.sumberDana.get.useQuery()
  const { mutateAsync, isLoading } = api.sumberDana.delete.useMutation()
    ...
    <Layout>
      <div className="container mx-auto mt-12">
        <div className="mb-5 space-y-1">
          <Breadcrumbs data={[{ name: "Sumber Dana" }]} />
          <p className="font-semibold">List Sumber Dana</p>
        </div>
        <DataTable
          data={!data ? [] : data}
          columns={[
            {
              ...
              header: () => 'KODE',
              cell: ({ row }: { row: any }) => (
                <Link href={`sumber-dana/${row.original.id}`} className="text-blue-500 hover:underline">
                  {row.original.kode}
                </Link>
              ),
            },
